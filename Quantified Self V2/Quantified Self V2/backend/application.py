...
def forgotpass1():
    ...
    if(user.query.filter_by(mail=mail).first() != None):
        ...
        file.close
        msg = MIMEMultipart()
        msg["From"] = 'quantifiedselfv2@gmail.com'
        msg["To"] = mail
        msg["Subject"] = "QuantifiedSelf - Reset Password"
        body = MIMEText("Use this OTP to reset your password: " + str(otp)+"\n Thank you!")
        msg.attach(body)

        with smtplib.SMTP("smtp.gmail.com") as connection:
                connection.starttls()
                connection.login(user='quantifiedselfv2@gmail.com', password='adlujgtvigksrqzy')
                connection.send_message(
                    msg=msg,
                    from_addr='quantifiedselfv2@gmail.com',
                    to_addrs=[mail],
                )
...
def export_tracker(id):
    ...
    for i in trackers:
        ...
        exportable.append(l)
    df=pd.DataFrame(exportable, columns=['Tracker Name','Tracker Description','Tracker Type','Tracker Settings','Date Created'])
    fp=open('yourtrack.csv','w')
    fp.close()
    df.to_csv('c:\\Users\\sam68\\OneDrive\\Desktop\\QuantifiedSelfV2\\Quantified Self V2\\backend\\yourtrack.csv',index=False)
    return send_file('yourtrack.csv')
...
def trackerinfo(uid,tid):
    ...
    if(len(data)!=0):
        if(tracker_info.tracker_type=='Numerical'):
            ...
            plt.ylabel('Value')
            plt.show()
            #plt.savefig('C:/Users/sam68/OneDrive/Desktop/Quantified Self V2/backend/static/plot.png',dpi=300)
            plt.close()
...
def monthly_report():
    with application.app_context():
        ...
        for user in users:
            uid = user.uid
            udata = user.query.filter_by(uid=uid).first()
