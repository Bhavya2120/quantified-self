<template>
  <h1 class="header">Tracker: {{name}}</h1>
...
<tr v-for="item,index in logs" :key="index">
                ...
                <th>{{item.note}}</th>
                <th>{{item.value}}</th>
                <th><button class="btn btn-outline-light"><router-link :to="`/updateLog/${item.logid}/${tid}`" style="text-decoration:strong; color: black;">Update</router-link></button>
                </th>
                <th>  <button class="btn btn-outline-dark">  <router-link :to="`/deleteLog/${item.logid}`" style="text-decoration:strong; color: red;">Delete</router-link></button></th>
